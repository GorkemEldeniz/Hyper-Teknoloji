...
>(({ className, ...props }, ref) => (
	<ToastPrimitives.Viewport
		ref={ref}
		className={cn(
			"fixed bottom-0 right-0 z-[100] flex max-h-screen w-full flex-col-reverse gap-2 p-4 sm:right-4 sm:bottom-4 sm:top-auto sm:flex-col md:max-w-[380px]",
			className
		)}
		{...props}
	/>
	...
	"group pointer-events-auto relative flex w-full items-center justify-between space-x-2 overflow-hidden rounded-lg border p-4 pr-6 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=open]:slide-in-from-bottom-full data-[state=closed]:slide-out-to-right-full",
	{
		variants: {
			variant: {
				...
				success:
					"border-green-200 bg-green-100 dark:bg-green-900/60 dark:border-green-800 text-green-900 dark:text-green-100",
				warning:
					"border-amber-200 bg-amber-100 dark:bg-amber-900/60 dark:border-amber-800 text-amber-900 dark:text-amber-100",
				info: "border-primary/30 bg-primary/10 text-primary dark:bg-primary/30 dark:border-primary/50",
			},
		},
		defaultVariants: {
			variant: "default",
...
>(({ className, ...props }, ref) => (
	<ToastPrimitives.Close
		ref={ref}
		className={cn(
			"absolute right-1 top-1 rounded-md p-1.5 text-foreground/40 opacity-0 transition-opacity hover:text-foreground focus:opacity-100 focus:outline-none focus:ring-1 group-hover:opacity-100 group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-50 group-[.destructive]:focus:ring-red-400 group-[.destructive]:focus:ring-offset-red-600 group-[.success]:text-green-900/80 group-[.success]:hover:text-green-900 group-[.warning]:text-amber-900/80 group-[.warning]:hover:text-amber-900 group-[.info]:text-primary/80 group-[.info]:hover:text-primary",
			className
		)}
		toast-close=''
		{...props}
